...
def register():
    if request.method == "POST":
        ...
        conn.execute(
            "INSERT INTO Guest (name, username, contact, passport, dob, pincode, password) VALUES (?, ?, ?, ?, ?, ?, ?)",
            (name, username, contact, passport, dob, pincode, password))
        conn.commit()
...
def booking():
    ...
    if request.method == "POST":
        ...
        return redirect(
            "/all-orders/" + "You have booked the room successfully, You can also self check in and check out")
    else:
...
def taxi_booking():
    ...
    if request.method == "POST":
        ...
        return jsonify({'redirect_url': "/all-orders/" + "You taxi booking request is noted. Our representative will contact you once the booking get confirmed after checking available cab options."})
    else:
...
def order_food():
    ...
    if request.method == "POST":
        ...
        conn.commit()
        return jsonify({'redirect_url': "/all-orders/"+"You food booking request is noted. Our representative will contact you once food will be ready to be delivered"})
    else:
    ...
    if not user_id:
        ...
        if room_id:
            if checkedIn == 0 and status=='active':
                finalurl = weburl + "/checkeIn/" + str(room_id) + "/" + str(user_id)
...
@app.route("/checkeIn/<int:room_id>/<int:user_id>", methods=["GET", "POST"])
